...
###
 ...
 # @LastEditors: Daniel
 # @LastEditTime: 2023-03-17 16:23:14
 # V2.0版本升级内容
 # 1 可以从a.txt读取设备清单，用户名，密码，特权密码 (可根据平台区域进行备份部署）
 ...
 # 2.8版
 # 1 备份生成的日志中对于设备上无法运行的检测命令会生成无效信息反馈，清理备份文件中的无效信息
 # 2 修复login认证失败适配不到思科的某些设备造成程序终止
 # 2.9版
 # 1 增加enable密码错误识别
 # 2 增加ip地址不可达的错误识别
 # 3.0
 # 1 修正了"Access denied"导致异常不继续执行未完成设备问题
### 
set timeout 120
#Read username, password, and enable password from a.txt
set fp [open "/zbq/a.txt" r]
...
set aa _
set current_folder ""
#Read device names, IP addresses, and per-device credentials from a.txt
set devices {}
set fp [open "/zbq/a.txt" r]
